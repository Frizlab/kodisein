...
void KPickHandler::removePickable( KPickable * pickable )
{
    ...
    PickableVector::iterator result = find(pickables.begin(), pickables.end(), pickable);
...
{
    if (flags[KDL_PICKHANDLER_FLAG_MOVING_ENABLED])
    {
        moved(mouseEvent);
        return true;
    }
...
void KPickHandler::updateProjection ( const KMouseEvent & mouseEvent )
{
    if (isProjectionEnabled())
    {
        ...
        updateProjectionFromEventInMode (projection, mouseEvent, KEventHandler::getMouseMode());
    }
}
