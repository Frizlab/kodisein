...
// --------------------------------------------------------------------------------------------------------
KTexture::KTexture ( const string & fn) : KObject ()
{
    texture_id	= 0;
    width 	= 0;
    height	= 0;
...
// --------------------------------------------------------------------------------------------------------
void KTexture::setFileName ( const string & fn )
{
    if (fn != filename)
    {
        filename = fn;
        ...
        if (surface)
        {
            ...
            if (width != surface->w || height != surface->h)
            {
                ...
                if (gluScaleImage(GL_RGBA, surface->w, surface->h, GL_UNSIGNED_INT_8_8_8_8,
                                    ...
                                    scaleImageBuffer) != 0)
                {
                    KConsole::printError(kStringPrintf("unable to scale texture file %s!", filename.c_str()));
