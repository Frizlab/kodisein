...
// --------------------------------------------------------------------------------------------------------
KFileNameField::KFileNameField ( const string & fn ) : KTextField (fn)
{
...
bool KFileNameField::handleKey ( const KKey & key )
{
    ...
    {
        string croppedPath = text.substr(0, cursor_pos); // path before cursor
        string restPath; 				 // path behind cursor
        
        // map cropped path to current directory and rest path to file prefix
        unsigned int lastSlashPos = croppedPath.rfind(kPathSep);
        ...
        string searchDir = kFileSubstitutePath(croppedPath); // replaces leading ~
        if (kFileIsAbsPath(searchDir) == false) // no absolute path -> prepend current_dir_path
        {
            searchDir = kFileJoinPaths(current_dir_path, searchDir);
        } 
        ...
        vector<string>::iterator iter = dir_entries.begin();
        ...
        else if (numMatches == 1) // exactly one match -> if directory, append slash
        {
            ...
            if (kFileIsDir(searchDir+matchingEntries[0]))
            {
                ...
                setText(croppedPath+matchingEntries[0]+kPathSep);
            }
            else
            {
        ...
        {
            string sharedPrefix = kStringGetSharedPrefix(matchingEntries);
            ...
            else
            {
                ...
                vector<string>::iterator lastPos;
                lastPos = find (matchingEntries.begin(), matchingEntries.end(), sharedPrefix+selection);
    ...
    else if (key.sym == SDLK_RETURN)
    {
        ...
        string currentPath = kFileSubstitutePath(text); // replaces leading ~
        if (kFileIsAbsPath(currentPath) == false) // no absolute path -> prepend current_dir_path
        {
            currentPath = kFileJoinPaths(current_dir_path, currentPath);
        } 
...
{
    unsigned int lastSlashPos = text.rfind(kPathSep);
    if (lastSlashPos == text.size()-1) lastSlashPos = text.rfind(kPathSep, lastSlashPos-1);
