...
void KWidgetArray::alignLabels ()
{
    ...
    vector<KLabel*>::iterator labelIter = labelWidgets.begin();
    while (labelIter != labelWidgets.end())
    {
        string labelText = (*labelIter)->getText();
        (*labelIter)->setText(labelText + string(maxLabelWidth - labelText.size(), ' '));
        labelIter++;
    }
}
