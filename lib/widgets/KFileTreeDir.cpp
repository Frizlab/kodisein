...
// --------------------------------------------------------------------------------------------------------
KFileTreeDir::KFileTreeDir ( const KFileTreeDir * d, const string & n ) : KFileTreeNode (n)
{
    parent = d;
    if (n.find("kodisein") != string::npos) 
    {
...
// --------------------------------------------------------------------------------------------------------
KFileTreeDir::KFileTreeDir ( const string & n ) : KFileTreeNode (n)
{
...
void KFileTreeDir::createTree ()
{
    ...
    string pathName = getAbsPathName();
    if (kFileGetDirEntries(pathName, entries, flags[KDL_FILETREEDIR_FLAG_SHOW_DOTFILES]))
    {
        ...
        {
            if (kFileIsDir(kFileJoinPaths(pathName, (*iter))))
            {
                addChild (new KFileTreeDir(this, (*iter)));
            }
            else
            {
                string suffix = kFileSuffix((*iter));
