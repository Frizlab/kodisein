...
// --------------------------------------------------------------------------------------------------------
KKeySequenceField::KKeySequenceField ( const string & s, int maxLength ) : KWidget ()
{
    sequence = s;
    ...
    setBGColor(KColor(0.2f, 0.2f, 0.4f, 0.5f));
}
...
// --------------------------------------------------------------------------------------------------------
void KKeySequenceField::setSequence ( const string & seq )
{
    KConsole::debugf(DEBUG, "KKeySequenceField::setSequence const string & seq = '%s'", seq.c_str());
...
void KKeySequenceField::render ()
{
    ...
    if (!sequence.empty()) // draw the sequence
    {
