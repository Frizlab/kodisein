...
// --------------------------------------------------------------------------------------------------------
void KTextWidget::setSize ( KSize newSize )
{   
    KLabel::setSize(newSize);
    return;
    if (newSize.w > size.w || newSize.h > size.h)
    {
        KSize maxSize(kMax(newSize.w, getColumnWidth() + 10.0), kMax(newSize.h, getRowHeight() + 5.0));
...
void KTextWidget::printf ( const char * fmt ... )
{
    ...
    string newText = kStringPrintFormArgs(fmt, &argList);
    va_end(argList);
