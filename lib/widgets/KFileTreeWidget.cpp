...
// --------------------------------------------------------------------------------------------------------
string KFileTreeWidget::getCurrentDir () const
{
    if (current_dir_node)
    {	
...
// --------------------------------------------------------------------------------------------------------
void KFileTreeWidget::setCurrentDir ( const string & dirName )
{
    string absDirPath;
    ...
    if (dirName[0] == '.' && current_dir_node) 
    {
        ...
        absDirPath = kFileJoinPaths(current_dir_node->getAbsPathName(), dirName);
    }
    else
    {
...
void KFileTreeWidget::refresh ()
{
    ...
    {
        string currentDirPath = current_dir_node->getAbsPathName();
        removeAllPickables();
        delete current_dir_node;
        current_dir_node = NULL;
...
// --------------------------------------------------------------------------------------------------------
void KFileTreeWidget::menuCallback ( const string & itemText )
{
...
void KFileTreeWidget::displayMenu ()
{
    if (popup_menu == NULL)
    {
        ...
        item->addReceiverCallback((KWidget*)this, 
                (KSetStringPtr)(void (KWidget::*)(const string &))&KFileTreeWidget::menuCallback );
        ...
        item->setProviderCallback((KWidget*)this, 
                (KGetBoolPtr)(bool (KWidget::*)() const)&KFileTreeWidget::getShowDotFiles );
        ...
        item->setProviderCallback((KWidget*)this, 
                (KGetBoolPtr)(bool (KWidget::*)() const)&KFileTreeWidget::getShowCubes );

        popup_menu->addMenuItem(item = new KMenuItem ("show texture(s)"));
        item->addReceiverCallback((KWidget*)this, (KSetStringPtr)(void (KWidget::*)(const string &))&KFileTreeWidget::menuCallback );
