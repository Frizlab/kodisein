...
{
    vector<KNamedReceiverCallbacks*>::const_iterator iter = named_receiver_callbacks.begin();
...
void KNotificationObject::removeReceiverCallback ( KObject * rec, KCallbackPtr cbPtr )
{
    ...
    vector<KReceiverCallback*>::iterator iter = namedReceiverCallbacks->receiver_callbacks.begin();
...
{
    vector<KNamedReceiverCallbacks*>::iterator iter = named_receiver_callbacks.begin();
    while (iter != named_receiver_callbacks.end())
    {
...
// --------------------------------------------------------------------------------------------------------
void removeReceiversInReceiverCallbackVector ( vector<KReceiverCallback*> * receiverCallbacks )
{
...
{ \
    ...
    vector<KReceiverCallback*>::const_iterator iter = namedReceiverCallbacks->receiver_callbacks.begin(); \
    while (iter != namedReceiverCallbacks->receiver_callbacks.end()) \
    { \
...
NOTIFY_RECEIVERS(const string &, KDL_NOTIFICATION_TYPE_SET_STRING,	set_string_ptr)
...
RECEIVE_VALUE(string, 		KDL_NOTIFICATION_TYPE_GET_STRING,	get_string_ptr)
