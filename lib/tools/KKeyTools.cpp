...
// --------------------------------------------------------------------------------------------------------
int kKeyGetDisplayWidthForPureKey ( const string & keyName )
{
...
// --------------------------------------------------------------------------------------------------------
int kKeyGetDisplayWidthForModifier ( const string & modName )
{    
    switch (modName[0])
    {
        case 'M': // META (APPLE)
...
// --------------------------------------------------------------------------------------------------------
int kKeyGetDisplayWidthForKey ( const string & keyName )
{
    ...
    string mod = keyName.substr(0, keyPos);
    string key = keyName.substr(keyPos+1);
    int width = 0;

    // width of the modifier symbols
    vector<string> modNames = kStringGetComponents(mod, "+");
    vector<string>::iterator iter = modNames.begin();
...
{
    int width = 0;
    vector<string> keyNames = kStringGetComponents(shortcut, " ");
    vector<string>::iterator iter = keyNames.begin();
...
// --------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------
int kKeyDisplayModifier ( const string & modName, const KPosition & pos )
{    
...
// --------------------------------------------------------------------------------------------------------
int kKeyDisplayPureKey ( const string & keyName, const KPosition & pos )
{
    KPosition start = pos;
    KPosition iconPos(pos.x, pos.y - KDL_MOD_ICON_SPACING);
    ...
    if (keySym > 32 && keySym < 127) // ascii characters
    {
        kDisplayString(string(1, keySym), pos);
        return KDL_MENU_CHARACTER_WIDTH;
    }
    else if (keySym >= 256 && keySym <= 272) // key pad keys
    {
...
// --------------------------------------------------------------------------------------------------------
void kKeyDisplayKeySequence ( const string & shortcut, const KPosition & pos )
{
    KPosition start = pos;
    vector<string> keyNames = kStringGetComponents(shortcut, " ");
    vector<string>::iterator iter = keyNames.begin();
...
// --------------------------------------------------------------------------------------------------------
SDLKey kKeyGetSymForSymName ( const string & keyName )
{
...
{
    string keyName = kKeyGetNameForSym(keySym.sym);
