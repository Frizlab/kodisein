...
// --------------------------------------------------------------------------------------------------------
string kXMLTag ( const string & name, const string & attributes, int depth )
{
    string xml(depth, ' ');
    xml += "<";
    xml += name;
...
// --------------------------------------------------------------------------------------------------------
string kXMLOpenTag ( const string & name, const string & attributes, int depth )
{
    string xml(depth, ' ');
    xml += "<";
    xml += name;
...
// --------------------------------------------------------------------------------------------------------
string kXMLCloseTag ( const string & name, int depth )
{
    string xml(depth, ' ');
    xml += "</";
    xml += name;
...
// --------------------------------------------------------------------------------------------------------
string kXMLPrintf ( int depth, const char * fmt ...)
{
    string text(depth, ' ');
    va_list argList;
    va_start(argList, fmt);
    text += kStringPrintFormArgs(fmt, &argList);
    va_end(argList);
    return text;
}

// --------------------------------------------------------------------------------------------------------
string kXMLParseToTagsInVector ( string & xml, const vector<string> & tags )
{
    string open("<");
    unsigned int minLoc = string::npos; 
    vector<string>::const_iterator iter = tags.begin();
    while (iter != tags.end())
    {
    ...
    string substring = xml.substr(0, minLoc);
    xml.erase(0, minLoc);
    return substring;
}

// --------------------------------------------------------------------------------------------------------
string kXMLReadNamedAttribute ( const string & xml, const string & name )
{
    string value;
    string nameStr(name);
    nameStr += "='";
...
// --------------------------------------------------------------------------------------------------------
bool kXMLParseNamedCloseTag ( string & xml, const string & name, bool printError )
{
...
// --------------------------------------------------------------------------------------------------------
bool kXMLReadNamedOpenTag ( const string & xml, const string & name, string * attributes )
{
    unsigned int loc = xml.find('<'), endloc;
    
    if (loc == string::npos || xml[loc+1] == '/') return false;
...
// --------------------------------------------------------------------------------------------------------
string kXMLParseNamedOpenTag ( string & xml, const string & name, string * attributes, bool printError )
{
    unsigned int loc = xml.find('<');
    if (loc == string::npos || xml[loc+1] == '/') 
    {
...
// --------------------------------------------------------------------------------------------------------
bool kXMLParseOpenTag ( string & xml, string & name, string * attributes, bool printError )
{
    unsigned int loc = xml.find('<');
    if (loc == string::npos || xml[loc+1] == '/') 
    {
...
// --------------------------------------------------------------------------------------------------------
string kXMLValue( const string & name, int type, const void * value, int depth )
{
    switch (type)
    {
        case KDL_XML_VALUE_FLOAT:	return kXMLTag("Value", kStringPrintf("name='%s' type='float' value='%g'", 
                                                name.c_str(), *(float *)value), depth);
        case KDL_XML_VALUE_INT:	return kXMLTag("Value", kStringPrintf("name='%s' type='int' value='%d'", 
                                                name.c_str(), *(int *)value), depth);
        case KDL_XML_VALUE_CHAR:	return kXMLTag("Value", kStringPrintf("name='%s' type='char' value='%c'", 
                                                name.c_str(), *(char *)value), depth);
        case KDL_XML_VALUE_BOOL:	return kXMLTag("Value", kStringPrintf("name='%s' type='bool' value='%s'", 
                                                name.c_str(), *(bool *)value ? "true" : "false"), depth);
        case KDL_XML_VALUE_VERTEX:	return kXMLTag("Value", kStringPrintf("name='%s' type='vector' x='%g' y='%g', z='%g'", 
                                                name.c_str(), 
                                                (*(KVector*)value)[0], 
                                                (*(KVector*)value)[1],
                                                (*(KVector*)value)[2]), depth);
        case KDL_XML_VALUE_SIZE:	return kXMLTag("Value", kStringPrintf("name='%s' type='size' w='%d' h='%d'", 
                                    name.c_str(), (*(KSize*)value).w, (*(KSize*)value).h), depth);
        case KDL_XML_VALUE_POSITION:return kXMLTag("Value", kStringPrintf("name='%s' type='position' x='%d' y='%d'", 
...
// --------------------------------------------------------------------------------------------------------
bool kXMLParseValue( string & xml, const string & name, int type, void * value, bool printError )
{
    unsigned int loc = xml.find('<');
    if (loc == string::npos || xml[loc+1] == '/')     
    {
    ...
    string saveString = xml.substr(0, loc);
    xml.erase(0, loc);
    ...
    string typeString = kXMLReadNamedAttribute(attributes, "type");
    ...
    {
        string substring = xml.substr(0, xml.find("</Value>"));
        ...
        else if (typeString == "string")
        {
            ...
            unsigned int last  = substring.rfind("\"", string::npos);
            *((string*)value) = substring.substr(first, last-first);
        }
        
        vector<string> tags(1, "/Value");
