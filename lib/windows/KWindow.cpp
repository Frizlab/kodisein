...
KWindow::KWindow ( int features ) : KPickHandler (), KPickable ()
{
    ...
    flags[KDL_WINDOW_FLAG_RESIZABLE] 		= (features & KDL_WINDOW_FEATURES_RESIZE_HANDLE) != 0;
    ...
    if (features & KDL_WINDOW_FEATURES_WIDGET)
    {
        ...
        widget->setFGColor (KColor(0.7f, 0.7f, 0.7f, 1.0f));
        widget->setBGColor (KColor(0.0f, 0.0f, 0.0f, 0.7f));
        ...
        if (features & KDL_WINDOW_FEATURES_BUTTON_WIDGET_BIT)
        {
            ...
            {
                close_button = new KIconButton(ICON_CROSS);
            ...
            {
                maximize_button = new KIconButton(ICON_ARROW_UP);
...
{
    if (maximize_button->getType() == ICON_ARROW_UP)
    {
        old_pos  = widget->getViewportPos();
        old_size = widget->getSize();
        
        setPosition (KPosition(76.0, 0));
        KSize viewportSize = projection->getViewportSize(); viewportSize.w -= 76;
...
void KWindow::display ( int )
{
    ...
    if (flags[KDL_WINDOW_FLAG_RESIZABLE] && widget->flags[KDL_WIDGET_FLAG_VISIBLE]) // draw the resize corner
    {
